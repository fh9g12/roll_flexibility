...
lg = legend('Free','Locked','Free (NAT)','Locked (NAT)');
...
name = NBNT_VLM.ChangeBaseClass("Roll_B3T2_FWT_RC");
tmp_res = res_free;
[~,idx] = min(([tmp_res.sweep]-(0)).^2);
% idx = length(res_locked);
...
function abs_plot(res,varargin)
    [~,idx] = unique([res.sweep]);
    tipDisp = [res(idx).TipDisp]./([res(idx).Span]./2)*100;
    idx2= abs(tipDisp)<=100 & abs([res(idx).Fold])<90;
    idx = idx(idx2)
    % idx2 = idx2 & [res(idx).sweep]>=-0.93;
    idx = idx(idx2);
    nexttile(1)
    plot([res(idx).sweep],[res(idx).RollRate],varargin{:})
    xlabel('$\sigma$')
    ylabel('Roll Rate [deg/s]')
    
    nexttile(2)
    plot([res(idx).sweep],[res(idx).TipTwist],varargin{:})
    xlabel('$\sigma$')
    ylabel('Tip Twist [deg]')
    
    nexttile(3)
    plot([res(idx).sweep],-[res(idx).TipDisp]./([res(idx).Span]./2)*100,varargin{:})
    xlabel('$\sigma$')
    ylabel('Tip Disp. [\% Semi-span]')
    ylim([-100,20])
    
    nexttile(4)
    plot([res(idx).sweep],[res(idx).Fold],varargin{:})
    xlabel('$\sigma$')
    ylabel('Fold [deg]')
end
...
arguments
    ...
    if ail~=0
        AilRes = model.pusedo_arclength_continuation(X0,params,2,sort([ail,0]),sign(ail),[1e-3,1],[0,inf],1,400,p_names,false,false,q_idx,out_idx);
    elseif torque~=0
        AilRes = model.pusedo_arclength_continuation(X0,params,3,sort([torque,0]),sign(torque),[1e-3,1],[0,inf],0.5,400,p_names,false,false,q_idx,out_idx);
    end
    AilRes = EnrichMeta(AilRes,model);
    if opts.IsRoll && ~isnan(opts.RollRate)
    ...
    res_up = model.pusedo_arclength_continuation(U,params,6,[0,sweep_max],1,[1e-3,inf],[0.01,0.5],0.05,600,p_names,false,false,q_idx,out_idx);
    % positive coupling sweep
    fh.printing.title(sprintf('Coupling sweeps Down'))
    res_dwn = model.pusedo_arclength_continuation(U,params,6,[-sweep_max,0],-1,[1e-3,inf],[0.01,0.5],0.05,600,p_names,false,false,q_idx,out_idx);

    res = [fliplr(res_dwn(2:end)),res_up];
